...
function BoardInner({ onLogout }: BoardPageProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // Precisamos carregar as frações para preencher corretamente o estado inicial
        ...
        replRows?.forEach((row: any) => {
          ...
          // row.replacement_fractions virá como array devido ao join
          ...
          upsertReplacement({
            ...
            list_id: null,
          }, fractions); // Passamos as frações aqui
        });
  ...
  useEffect(() => {
    // Canal único para ouvir todas as tabelas relevantes
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          if (
            ...
            // Em updates realtime, não recebemos as frações no payload.new
            // O upsertReplacement lida com isso mantendo as existentes se o argumento for undefined
      ...
      .on(
        ...
        (payload) => {
          // Escuta inserções de frações (ex: +10 min)
          if (
            payload.eventType === "INSERT" ||
            payload.eventType === "UPDATE"
          ) {
          ...
          // Delete de fração não implementado no store ainda, mas poderia ser aqui
        }
      )
      .subscribe();
